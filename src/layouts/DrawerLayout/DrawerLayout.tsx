import { Close } from "@mui/icons-material";
import type { DrawerProps } from "@mui/material/Drawer";
import type { ReactNode } from "react";

import {
  DialogActions,
  DialogContent,
  DialogTitle,
  Drawer,
  IconButton,
} from "@mui/material";
import type { SxProps, Theme } from "@mui/material/styles";

const NARROW_WIDTH = 450;

type DrawerMode = "narrow" | "wide" | "fullscreen";

const modeWidth: Record<DrawerMode, number | string | undefined> = {
  narrow: NARROW_WIDTH,
  wide: undefined,
  fullscreen: "100vw",
};

interface DrawerLayoutProps extends Omit<DrawerProps, "title"> {
  open: boolean;
  onClose: () => void;
  children?: ReactNode;
  title?: ReactNode;
  extraActions?: ReactNode;
  sx?: SxProps<Theme>;
  contentSx?: SxProps<Theme>;
  paperSx?: SxProps<Theme>;
  titleSx?: SxProps<Theme>;
  mode?: DrawerMode;
  disableEnforceFocus?: boolean;
}

const DrawerLayout = ({
  open,
  onClose,
  children,
  title,
  extraActions,
  sx,
  contentSx,
  paperSx,
  titleSx,
  mode = "fullscreen",
  disableEnforceFocus = true,
  slotProps: slotPropsProp,
  ...rest
}: DrawerLayoutProps) => {
  const width = modeWidth[mode];

  return (
    <Drawer
      anchor="right"
      open={open}
      onClose={onClose}
      sx={sx}
      disableEnforceFocus={disableEnforceFocus}
      {...rest}
      slotProps={{
        ...(slotPropsProp ?? {}),
        paper: {
          sx: {
            backgroundColor: (theme: Theme) =>
              `${theme.palette.background.default} !important`,
            display: "flex",
            flexDirection: "column",
            maxWidth: "100vw",
            width: width ?? { xs: "100vw", sm: "90vw", md: "85vw" },
            transition: "width 0.4s cubic-bezier(0.22, 1, 0.36, 1)",
            "@keyframes slideInFromRight": {
              from: { transform: "translateX(100%)" },
              to: { transform: "translateX(0)" },
            },
            animation: open
              ? "slideInFromRight 400ms cubic-bezier(0.22, 1, 0.36, 1) forwards"
              : "none",
            ...(paperSx ?? {}),
          },
        },
      }}
    >
      {title && (
        <DialogTitle sx={{ flexShrink: 0, ...(titleSx ?? {}) }}>
          {title}
        </DialogTitle>
      )}

      <DialogContent sx={{ flex: 1, overflow: "auto", ...(contentSx ?? {}) }}>
        {children}
      </DialogContent>

      <DialogActions sx={{ position: "absolute", top: -10, right: 0 }}>
        {extraActions}
        <IconButton onClick={onClose} size="small">
          <Close />
        </IconButton>
      </DialogActions>
    </Drawer>
  );
};

export default DrawerLayout;
