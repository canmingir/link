...
import type { FlowNodeMap, FlowViewportHandle } from "../lib/Flow/types";
...
const treeToLinked = (
  tree: DemoTree | null | undefined
): { nodes: FlowNodeMap; roots: string[] } => {
  if (!tree) return { nodes: {}, roots: [] };

  const nodes: FlowNodeMap = {};
  const roots: string[] = [];

  const walk = (node: DemoTree | undefined, parentId: string | null = null) => {
    ...
    kids.forEach((child: DemoTree) => walk(child, node.id));
  };
...
export const FitViewDemo = {
  render: () => {
    const flowRef = useRef<FlowViewportHandle>(null);
    ...
    return (
      <Stack sx={{ height: "100vh" }}>
        <Stack direction="row" spacing={1} sx={{ p: 1 }}>
          ...
          <Button variant="outlined" onClick={() => flowRef.current?.zoomOut()}>
            Zoom Out
          </Button>
        </Stack>
        <Flow
...
export const FitViewOnNodesChangeDemo = {
  render: () => {
    const flowRef = useRef<FlowViewportHandle>(null);
    const [tree, setTree] = useState<DemoTree>(wideTree);
    const data = treeToLinked(tree);

    const addLeafToBranch1 = () => {
      setTree((prev) => {
        const branch1 = prev.children?.find((c) => c.id === "branch1");
        const nextIndex = (branch1?.children?.length ?? 0) + 1;
        const newLeaf: DemoTree = {
    ...
    return (
      <Stack sx={{ height: "100vh" }}>
        <Stack direction="row" spacing={1} sx={{ p: 1 }}>
          ...
          <Button variant="outlined" onClick={() => flowRef.current?.fitView()}>
            Fit View
          </Button>
        </Stack>
        <Flow
