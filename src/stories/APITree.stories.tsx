...
import type { ApiEndpoint } from "../lib/APITree/APITree";
import type { ApiType } from "../lib/APITypes/types";
import CssBaseline from "@mui/material/CssBaseline";
import NewAPIBody from "../lib/NewApiBody/NewAPIBody";
import type { Param } from "../lib/ParamTable/types";
import type { SchemaEditorHandle } from "../lib/SchemaEditor/SchemaEditor";
...
const APIWorkspace = () => {
  ...
  const paramsRef = useRef<Param[]>([]);
  const addParams = useRef<(() => void) | null>(null);
  const requestSchemaRef = useRef<SchemaEditorHandle | null>(null);
  const responseSchemaRef = useRef<SchemaEditorHandle | null>(null);
  const typesRef = useRef<SchemaEditorHandle | null>(null);
  ...
  const [typeAdd] = useEvent<{ typeName?: string } | null>(
    "API_TYPE_ADD",
    null
  );
  const [typeDelete] = useEvent<{ typeName?: string } | null>(
    "API_TYPE_DELETE",
    null
  );
  const [typeRename] = useEvent<{
    oldTypeName?: string;
    newTypeName?: string;
  } | null>("API_TYPE_RENAME", null);

  const [methodDelete] = useEvent<{ type?: string; action?: string } | null>(
    "API_METHOD_DELETE",
    null
  );
  const [resourceDelete] = useEvent<{ path?: string } | null>(
    "API_RESOURCE_DELETE",
    null
  );
  const [apiDialogOpen] = useEvent<{ type?: string; action?: string } | null>(
    "API_DIALOG_OPEN",
    null
  );
      ...
      prev.map(
        (t): ApiType =>
          t.name === typeRename.oldTypeName
            ? {
                ...t,
                name: typeRename.newTypeName ?? t.name,
                schema: {
                  type: "object",
                  ...t.schema,
  ...
  useEffect(() => {
    ...
    setDialog((prev) => ({
      ...prev,
      type: apiDialogOpen.type ?? null,
      action: apiDialogOpen.action ?? null,
      open: true,
    }));
  }, [apiDialogOpen]);
    ...
    if (isEdit) {
      setApi((prev) => {
        ...
        next[idx] = {
          ...
          response: {
            ...(current.response as Record<string, unknown> | undefined),
