...
const pkg: { name: string; version: string; description: string } = {
  ...
  description: (pkgJson as { description?: string }).description ?? "",
};
...
const SettingsDialogTabs = ({
  tabs,
}: {
  tabs?: Array<{ label: string; panel: React.ComponentType }>;
}) => {
        ...
        {users?.map((user) => (
          ...
          >
            ...
            <ListItemSecondaryAction>
              <IconButton
                onClick={async () => {
                  if (user.id) await deletePermission(user.id);
                }}
                sx={{ color: "error.main" }}
              >
