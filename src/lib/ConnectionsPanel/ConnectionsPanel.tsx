...
})<{ align?: Align; bound?: "true" | "false" }>(({ align, bound, theme }) => ({
  flex: 1,
  minWidth: 0,
  fontSize: 11,
...
type ConnectionField = {
  ...
  onHandleMouseDown?: MouseEventHandler<HTMLDivElement>;
};

type ConnectionsPanelProps = {
  inputs?: ConnectionField[];
  outputs?: ConnectionField[];
  inputsLabel?: ReactNode;
  outputsLabel?: ReactNode;
  renderLabel?: (name: string, side: ConnectionSide) => ReactNode;
  nodeId?: string;
  sx?: SxProps<Theme>;
};
...
const ConnectionsPanel = ({
  ...
  const label = (name: string, side: ConnectionSide) =>
    renderLabel ? renderLabel(name, side) : name;

  const attrsFor = (side: ConnectionSide, field: ConnectionField) => {
