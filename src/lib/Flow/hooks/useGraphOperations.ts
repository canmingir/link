...
import type { FlowNode, FlowNodeMap, FlowStructure } from "../types";
import {
  addToNext,
  cleanupReferences,
  collectSubtree,
...
/** A detached (copy/paste) sub-graph living outside the main tree. */
export type FloatingStructure = FlowStructure & {
  _pasteId?: string;
  id?: string;
  key?: string;
};
...
interface UseGraphOperationsParams {
  ...
  setFloatingNodes: Dispatch<SetStateAction<FloatingStructure[]>>;
  editable?: boolean;
}
...
export const useGraphOperations = ({
      ...
      if (isInFloating) {
        setFloatingNodes((prev) =>
          prev
            ...
            .filter((s): s is FloatingStructure => Boolean(s))
        );
      }
    },
  ...
  const handlePaste = useCallback(
    (structure: FloatingStructure) => {
      ...
      const internalChildIds = new Set<string>();

      validNodeIds.forEach((id) => {
        const node = validNodes[id];
        const raw = node.raw as { next?: FlowNode["next"] } | undefined;

        const nextIds = toNextArray(node.next || raw?.next);
  ...
  const handleConnect = useCallback(
    (targetNodeId: string, selectedIds: string[]) => {
      if (!editable || !onChange || !nodesById || !selectedIds?.length) return;
