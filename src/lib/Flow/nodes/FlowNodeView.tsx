...
import type { FlowStyle } from "../styles";
...
import type { FlowNode as FlowNodeType, Point } from "../types";
import type { FlowPluginArg, FlowStyleResolver } from "../hooks/useNodeStyle";
import { useEffect, useLayoutEffect, useRef, useState } from "react";

export type FlowNodeData = FlowNodeType & {
...
interface FlowNodeViewProps {
  ...
  isRoot?: boolean;
  /** FlowNode forwards unmatched props through here. */
  [key: string]: unknown;
}
...
const FlowNodeView = ({
  ...
  const [childElList, setChildElList] = useState<HTMLElement[]>([]);

  const [connectorTick, setConnectorTick] = useState(0);

  const handleDrag = (newOffset?: Point) => {
      ...
      {hasChildren && (
        <>
          {(node.children ?? []).map((child, index) => {
            ...
            const gradient =
              childEdgeProps.gradient &&
              typeof childEdgeProps.gradient === "object"
                ? childEdgeProps.gradient
                : null;
            ...
            return (
              <DynamicConnector
                ...
                label={childEdgeProps.label as ReactNode}
