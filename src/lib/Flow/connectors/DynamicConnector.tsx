...
const DynamicConnector = ({
  ...
  startGap = 8,
  endGap = 10,
}: DynamicConnectorProps) => {
  const uniqueId = useId();
  const [dims, setDims] = useState<ConnectorDims | null>(null);
  const [points, setPoints] = useState<ConnectorPoints>({
    parent: null,
    children: [],
  });
  ...
  useLayoutEffect(() => {
    ...
    const update = () => {
      ...
      if (isHorizontal) {
        ...
        childPoints = childEls.map((el: HTMLElement | null) => {
    ...
    ro.observe(parentEl);
    childEls.forEach((el: HTMLElement | null) => el && ro.observe(el));
  ...
  const parentPointResolved = points.parent;
    ...
    <>
      ...
      >
        ...
        {points.children.map((child, i) => {
          const pathD = getPath(parentPointResolved, child);
      ...
      {label &&
        points.children.map((child, i) => {
          const labelX =
            parentPointResolved.x +
            (child.x - parentPointResolved.x) * labelPosition +
            labelOffsetX;
          const labelY =
            parentPointResolved.y +
            (child.y - parentPointResolved.y) * labelPosition +
            labelOffsetY;
