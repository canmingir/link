import { DataGrid } from "@mui/x-data-grid";
import { HighlightOff } from "@mui/icons-material";
import TypeMenu from "./TypeMenu";
import styles from "./styles";

import { Checkbox, IconButton, InputBase } from "@mui/material";
import type { Dispatch, SetStateAction } from "react";
import type {
  GridColDef,
  GridRenderCellParams,
  GridValueGetter,
} from "@mui/x-data-grid";
import type { NamedType, Param } from "./types";

interface ParamTableProps {
  types: NamedType[];
  params: Param[];
  setParams: Dispatch<SetStateAction<Param[]>>;
}

const ParamTable = ({ types, params, setParams }: ParamTableProps) => {
  const columns: GridColDef<Param>[] = [
    {
      field: "name",
      headerName: "Name",
      renderCell: (param: GridRenderCellParams<Param>) => {
        const { id } = param.row;
        return (
          <InputBase
            disabled={param.row.in === "path"}
            value={param.value || ""}
            onChange={(event) => updateParam(id, "name", event.target.value)}
            fullWidth
            placeholder="Enter name"
            data-cy={`param-name-field-${id}`}
          />
        );
      },
      flex: 1,
    },
    {
      field: "type",
      headerName: "Type",
      renderCell: (param: GridRenderCellParams<Param>) => {
        const { id } = param.row;
        return (
          <TypeMenu
            disabled={param.row.in === "path"}
            primitive
            types={types}
            id={id}
            type={param.value}
            onChange={(type: string) => updateParam(id, "type", type)}
          />
        );
      },
      flex: 1,
    },
    {
      field: "in",
      headerName: "Param",
      flex: 1,
      valueGetter: ((value: unknown) =>
        value || "query") as GridValueGetter<Param>,
    },
    {
      field: "required",
      headerName: "Required",
      renderCell: (param: GridRenderCellParams<Param>) => {
        const { id } = param.row;
        return (
          <Checkbox
            disabled={param.row.in === "path"}
            checked={param.value}
            onChange={(event) =>
              updateParam(id, "required", event.target.checked)
            }
            data-cy={`param-required-checkbox-${id}`}
          />
        );
      },
      flex: 1,
    },
    {
      field: "description",
      headerName: "Description",
      renderCell: (param: GridRenderCellParams<Param>) => {
        const { id } = param.row;
        return (
          <InputBase
            disabled={param.row.in === "path"}
            value={param.value || ""}
            onChange={(event) =>
              updateParam(id, "description", event.target.value)
            }
            fullWidth
            placeholder="Enter description"
            data-cy={`param-description-field-${id}`}
          />
        );
      },
      flex: 2,
    },
    {
      field: "action",
      headerName: " ",
      sortable: false,
      renderCell: (param: GridRenderCellParams<Param>) => {
        const { id } = param.row;
        return (
          <IconButton
            disabled={param.row.in === "path"}
            onClick={() => removeParam(id)}
            size="large"
          >
            <HighlightOff />
          </IconButton>
        );
      },
    },
  ];

  const removeParam = (id: string) => {
    setParams((prevParams) => prevParams.filter((param) => param.id !== id));
  };

  const updateParam = (id: string, field: string, value: unknown) => {
    setParams((prevParams) =>
      prevParams.map((param) =>
        param.id === id ? { ...param, [field]: value } : param
      )
    );
  };

  return (
    <DataGrid sx={styles.dataGrid} columns={columns} rows={params} hideFooter />
  );
};

export default ParamTable;
