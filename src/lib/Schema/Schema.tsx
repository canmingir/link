...
interface SchemaProps {
  initialData?: Partial<SchemaNode> | null;
  customTypes?: CustomType[];
}

const Schema = ({ initialData = {}, customTypes = [] }: SchemaProps) => {
  const [schemaData, setSchemaData] = useState<SchemaNode>({} as SchemaNode);

  useEffect(() => {
    const addIdsToSchema = (
    ...
    if (!initialData || Object.keys(initialData).length === 0) {
      ...
      const withIds = addIdsToSchema(initialData as SchemaNode);
      if (withIds) setSchemaData(withIds);
    }
  }, [initialData]);

  const renderTree = (node: SchemaNode, level = 0): ReactNode => (
    <TreeItem
      key={node.id}
